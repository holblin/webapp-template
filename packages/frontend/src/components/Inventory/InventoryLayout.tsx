import {
  ActionButton,
  Button,
  ButtonGroup,
  Cell,
  Column,
  Content,
  Dialog,
  DialogContainer,
  Divider,
  Heading,
  Row,
  SearchField,
  TableBody,
  TableHeader,
  TableView,
  type SortDescriptor,
} from '@react-spectrum/s2';
import { style } from '@react-spectrum/s2/style' with { type: 'macro' };
import type { ReactNode } from 'react';
import { useState } from 'react';
import { useIsSmallViewport } from 'src/components/Responsive/useIsSmallViewport';

export type InventoryLayoutColumn<TRow extends object> = {
  id: string;
  header: ReactNode;
  isRowHeader?: boolean;
  allowsSorting?: boolean;
  sortField?: string;
  allowsResizing?: boolean;
  renderCell: (row: TRow) => ReactNode;
};

export type InventoryLayoutState = {
  offset: number;
  search: string;
  showFilters: boolean;
};

export type InventoryLayoutProps<TRow extends object, TState extends InventoryLayoutState, TFilters extends Record<string, unknown>> = {
  title: string;
  totalCount: number;
  state: TState;
  onStateChange: (nextState: TState) => void;
  topRight?: ReactNode;
  columns: InventoryLayoutColumn<TRow>[];
  rows: TRow[];
  getRowId: (row: TRow) => string;
  ariaLabel: string;
  getFilters: (state: TState) => TFilters;
  getActiveFilters?: (state: TState, filters: TFilters) => Array<{
    id: string;
    label: string;
    patch: Partial<TState>;
  }>;
  clearFiltersPatch?: Partial<TState>;
  renderFilterPanel: (filters: TFilters, onFiltersChange: (patch: Partial<TFilters>) => void) => ReactNode;
  loadingState?: 'loading' | 'loadingMore' | 'sorting' | 'filtering' | 'idle';
  onLoadMore?: () => void;
  sortDescriptor?: SortDescriptor;
  onSortChange?: (
    descriptor: SortDescriptor,
    column: InventoryLayoutColumn<TRow> | undefined,
    onStatePatch: (patch: Partial<TState>, resetOffset?: boolean) => void,
  ) => void;
};

type InventoryStatePatchFn<TState extends InventoryLayoutState> = (patch: Partial<TState>, resetOffset?: boolean) => void;

type ActiveFilter<TState extends InventoryLayoutState> = {
  id: string;
  label: string;
  patch: Partial<TState>;
};

type InventoryTableProps<TRow extends object, TState extends InventoryLayoutState> = {
  columns: InventoryLayoutColumn<TRow>[];
  rows: TRow[];
  getRowId: (row: TRow) => string;
  ariaLabel: string;
  loadingState: 'loading' | 'loadingMore' | 'sorting' | 'filtering' | 'idle';
  onLoadMore?: () => void;
  sortDescriptor?: SortDescriptor;
  onSortChange?: (
    descriptor: SortDescriptor,
    column: InventoryLayoutColumn<TRow> | undefined,
    onStatePatch: InventoryStatePatchFn<TState>,
  ) => void;
  onStatePatch: InventoryStatePatchFn<TState>;
};

const InventoryTable = <TRow extends object, TState extends InventoryLayoutState>({
  columns,
  rows,
  getRowId,
  ariaLabel,
  loadingState,
  onLoadMore,
  sortDescriptor,
  onSortChange,
  onStatePatch,
}: InventoryTableProps<TRow, TState>) => {
  return (
    <div
      className={style({
        flexGrow: 1,
        minWidth: 0,
        minHeight: 0,
        display: 'flex',
        flexDirection: 'column',
        overflow: 'hidden',
      })}
    >
      <div
        className={style({ display: 'flex', flexDirection: 'column', gap: 8, flexGrow: 1, minHeight: 0 })}
      >
        <TableView
          aria-label={ariaLabel}
          styles={style({ width: 'full', height: 'full' })}
          loadingState={loadingState}
          onLoadMore={onLoadMore}
          sortDescriptor={sortDescriptor}
          onSortChange={onSortChange
            ? (descriptor) => {
              const column = columns.find((entry) => entry.id === String(descriptor.column));
              onSortChange(descriptor, column, onStatePatch);
            }
            : undefined}
        >
          <TableHeader>
            {columns.map((column) => (
              <Column
                key={column.id}
                id={column.id}
                isRowHeader={column.isRowHeader}
                allowsSorting={column.allowsSorting ?? Boolean(column.sortField)}
                allowsResizing={column.allowsResizing ?? true}
              >
                {column.header}
              </Column>
            ))}
          </TableHeader>
          <TableBody items={rows}>
            {(row) => (
              <Row id={getRowId(row)}>
                {columns.map((column) => (
                  <Cell key={column.id}>{column.renderCell(row)}</Cell>
                ))}
              </Row>
            )}
          </TableBody>
        </TableView>
      </div>
    </div>
  );
};

type InventoryActiveFiltersProps<TState extends InventoryLayoutState> = {
  isPhoneViewport: boolean;
  activeFilters: ActiveFilter<TState>[];
  clearFiltersPatch?: Partial<TState>;
  onStatePatch: InventoryStatePatchFn<TState>;
};

const InventoryActiveFilters = <TState extends InventoryLayoutState>({
  isPhoneViewport,
  activeFilters,
  clearFiltersPatch,
  onStatePatch,
}: InventoryActiveFiltersProps<TState>) => {
  if (activeFilters.length === 0) {
    return null;
  }

  return (
    <div
      className={isPhoneViewport
        ? style({
          display: 'flex',
          alignItems: 'center',
          gap: 8,
          flexShrink: 0,
          flexWrap: 'nowrap',
          overflowX: 'auto',
          overflowY: 'hidden',
        })
        : style({
          display: 'flex',
          alignItems: 'center',
          gap: 8,
          flexShrink: 0,
          flexWrap: 'wrap',
        })}
    >
      {activeFilters.map((activeFilter) => (
        <div key={activeFilter.id} className={style({ flexShrink: 0 })}>
          <ActionButton onPress={() => onStatePatch(activeFilter.patch, true)}>
            {activeFilter.label} ✕
          </ActionButton>
        </div>
      ))}
      {clearFiltersPatch ? (
        <div className={style({ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 })}>
          <Divider orientation="vertical" />
          <ActionButton onPress={() => onStatePatch(clearFiltersPatch, true)}>
            Clear all
          </ActionButton>
        </div>
      ) : null}
    </div>
  );
};

type InventoryPhoneLayoutProps<
  TRow extends object,
  TState extends InventoryLayoutState,
  TFilters extends Record<string, unknown>,
> = {
  title: string;
  totalCount: number;
  state: TState;
  topRight?: ReactNode;
  filters: TFilters;
  activeFilters: ActiveFilter<TState>[];
  clearFiltersPatch?: Partial<TState>;
  renderFilterPanel: (filters: TFilters, onFiltersChange: (patch: Partial<TFilters>) => void) => ReactNode;
  onStatePatch: InventoryStatePatchFn<TState>;
  columns: InventoryLayoutColumn<TRow>[];
  rows: TRow[];
  getRowId: (row: TRow) => string;
  ariaLabel: string;
  loadingState: 'loading' | 'loadingMore' | 'sorting' | 'filtering' | 'idle';
  onLoadMore?: () => void;
  sortDescriptor?: SortDescriptor;
  onSortChange?: (
    descriptor: SortDescriptor,
    column: InventoryLayoutColumn<TRow> | undefined,
    onStatePatch: InventoryStatePatchFn<TState>,
  ) => void;
};

const InventoryPhoneLayout = <
  TRow extends object,
  TState extends InventoryLayoutState,
  TFilters extends Record<string, unknown>,
>({
  title,
  totalCount,
  state,
  topRight,
  filters,
  activeFilters,
  clearFiltersPatch,
  renderFilterPanel,
  onStatePatch,
  columns,
  rows,
  getRowId,
  ariaLabel,
  loadingState,
  onLoadMore,
  sortDescriptor,
  onSortChange,
}: InventoryPhoneLayoutProps<TRow, TState, TFilters>) => {
  const [isFilterDialogOpen, setIsFilterDialogOpen] = useState(false);
  const filterButtonLabel = activeFilters.length > 0
    ? `Filters (${activeFilters.length})`
    : 'Filters';

  return (
    <div
      className={style({
        padding: 12,
        display: 'flex',
        flexDirection: 'column',
        gap: 12,
        flexGrow: 1,
        minHeight: 0,
      })}
    >
      <h1 className={style({ marginY: 0, font: 'heading-lg' })}>{title}</h1>

      <div
        className={style({
          display: 'flex',
          alignItems: 'center',
          flexShrink: 0,
          flexWrap: 'nowrap',
          overflowX: 'auto',
          overflowY: 'hidden',
        })}
      >
        <div className={style({ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 })}>
          <ActionButton onPress={() => setIsFilterDialogOpen(true)}>{filterButtonLabel}</ActionButton>
          <div className={style({ color: 'gray-700', font: 'body-sm' })}>{totalCount} item(s)</div>
        </div>
        {topRight ? (
          <div className={style({ marginStart: 'auto', paddingStart: 12, flexShrink: 0 })}>
            {topRight}
          </div>
        ) : null}
      </div>

      <InventoryActiveFilters<TState>
        isPhoneViewport
        activeFilters={activeFilters}
        clearFiltersPatch={clearFiltersPatch}
        onStatePatch={onStatePatch}
      />

      <div className={style({ display: 'flex', gap: 12, flexGrow: 1, minHeight: 0 })}>
        <InventoryTable<TRow, TState>
          columns={columns}
          rows={rows}
          getRowId={getRowId}
          ariaLabel={ariaLabel}
          loadingState={loadingState}
          onLoadMore={onLoadMore}
          sortDescriptor={sortDescriptor}
          onSortChange={onSortChange}
          onStatePatch={onStatePatch}
        />
      </div>

      <DialogContainer onDismiss={() => setIsFilterDialogOpen(false)}>
        {isFilterDialogOpen ? (
          <Dialog>
            {({ close }) => (
              <>
                <Heading slot="title">Filters</Heading>
                <Content>
                  <div className={style({ display: 'flex', flexDirection: 'column', gap: 12 })}>
                    <SearchField
                      aria-label={`Search ${title}`}
                      placeholder={`Search ${title.toLowerCase()}`}
                      value={state.search}
                      onChange={(value) => onStatePatch({ search: value } as Partial<TState>, true)}
                    />
                    {renderFilterPanel(filters, (patch) => onStatePatch(patch as Partial<TState>, true))}
                  </div>
                </Content>
                <ButtonGroup>
                  <Button
                    variant="secondary"
                    onPress={() => {
                      close();
                      setIsFilterDialogOpen(false);
                    }}
                  >
                    Close
                  </Button>
                </ButtonGroup>
              </>
            )}
          </Dialog>
        ) : null}
      </DialogContainer>
    </div>
  );
};

type InventoryDesktopLayoutProps<
  TRow extends object,
  TState extends InventoryLayoutState,
  TFilters extends Record<string, unknown>,
> = {
  title: string;
  totalCount: number;
  state: TState;
  topRight?: ReactNode;
  filters: TFilters;
  activeFilters: ActiveFilter<TState>[];
  clearFiltersPatch?: Partial<TState>;
  renderFilterPanel: (filters: TFilters, onFiltersChange: (patch: Partial<TFilters>) => void) => ReactNode;
  onStatePatch: InventoryStatePatchFn<TState>;
  columns: InventoryLayoutColumn<TRow>[];
  rows: TRow[];
  getRowId: (row: TRow) => string;
  ariaLabel: string;
  loadingState: 'loading' | 'loadingMore' | 'sorting' | 'filtering' | 'idle';
  onLoadMore?: () => void;
  sortDescriptor?: SortDescriptor;
  onSortChange?: (
    descriptor: SortDescriptor,
    column: InventoryLayoutColumn<TRow> | undefined,
    onStatePatch: InventoryStatePatchFn<TState>,
  ) => void;
};

const InventoryDesktopLayout = <
  TRow extends object,
  TState extends InventoryLayoutState,
  TFilters extends Record<string, unknown>,
>({
  title,
  totalCount,
  state,
  topRight,
  filters,
  activeFilters,
  clearFiltersPatch,
  renderFilterPanel,
  onStatePatch,
  columns,
  rows,
  getRowId,
  ariaLabel,
  loadingState,
  onLoadMore,
  sortDescriptor,
  onSortChange,
}: InventoryDesktopLayoutProps<TRow, TState, TFilters>) => {
  const isInlineFilterVisible = state.showFilters;

  return (
    <div
      className={style({
        padding: 20,
        display: 'flex',
        flexDirection: 'column',
        gap: 12,
        flexGrow: 1,
        minHeight: 0,
      })}
    >
      <h1 className={style({ marginY: 0, font: 'heading-lg' })}>{title}</h1>

      <div className={style({ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexShrink: 0 })}>
        <div className={style({ display: 'flex', alignItems: 'center', gap: 8 })}>
          <ActionButton onPress={() => onStatePatch({ showFilters: !state.showFilters } as Partial<TState>)}>
            {state.showFilters ? 'Hide filters' : 'Show filters'}
          </ActionButton>
          <SearchField
            aria-label={`Search ${title}`}
            placeholder={`Search ${title.toLowerCase()}`}
            value={state.search}
            onChange={(value) => onStatePatch({ search: value } as Partial<TState>, true)}
            styles={style({ width: 320 })}
          />
          <div className={style({ color: 'gray-700', font: 'body-sm' })}>{totalCount} item(s)</div>
        </div>
        {topRight}
      </div>

      <InventoryActiveFilters<TState>
        isPhoneViewport={false}
        activeFilters={activeFilters}
        clearFiltersPatch={clearFiltersPatch}
        onStatePatch={onStatePatch}
      />

      <div className={style({ display: 'flex', gap: 12, flexGrow: 1, minHeight: 0 })}>
        {isInlineFilterVisible ? (
          <div
            className={style({
              width: 280,
              flexShrink: 0,
              borderWidth: 1,
              borderColor: 'gray-300',
              borderStyle: 'solid',
              borderRadius: 'lg',
              padding: 12,
              overflow: 'auto',
              backgroundColor: 'gray-50',
            })}
          >
            {renderFilterPanel(filters, (patch) => onStatePatch(patch as Partial<TState>, true))}
          </div>
        ) : null}

        <InventoryTable<TRow, TState>
          columns={columns}
          rows={rows}
          getRowId={getRowId}
          ariaLabel={ariaLabel}
          loadingState={loadingState}
          onLoadMore={onLoadMore}
          sortDescriptor={sortDescriptor}
          onSortChange={onSortChange}
          onStatePatch={onStatePatch}
        />
      </div>
    </div>
  );
};

export const InventoryLayout = <
  TRow extends object,
  TState extends InventoryLayoutState,
  TFilters extends Record<string, unknown>,
>({
  title,
  totalCount,
  state,
  onStateChange,
  topRight,
  columns,
  rows,
  getRowId,
  ariaLabel,
  getFilters,
  getActiveFilters,
  clearFiltersPatch,
  renderFilterPanel,
  loadingState = 'idle',
  onLoadMore,
  sortDescriptor,
  onSortChange,
}: InventoryLayoutProps<TRow, TState, TFilters>) => {
  const isPhoneViewport = useIsSmallViewport(680);

  const onStatePatch: InventoryStatePatchFn<TState> = (patch, resetOffset = false) => {
    const nextState: TState = {
      ...state,
      ...patch,
      offset: resetOffset ? 0 : (patch.offset ?? state.offset),
    };

    onStateChange(nextState);
  };

  const filters = getFilters(state);
  const activeFilters = getActiveFilters ? getActiveFilters(state, filters) : [];

  if (isPhoneViewport) {
    return (
      <InventoryPhoneLayout<TRow, TState, TFilters>
        title={title}
        totalCount={totalCount}
        state={state}
        topRight={topRight}
        filters={filters}
        activeFilters={activeFilters}
        clearFiltersPatch={clearFiltersPatch}
        renderFilterPanel={renderFilterPanel}
        onStatePatch={onStatePatch}
        columns={columns}
        rows={rows}
        getRowId={getRowId}
        ariaLabel={ariaLabel}
        loadingState={loadingState}
        onLoadMore={onLoadMore}
        sortDescriptor={sortDescriptor}
        onSortChange={onSortChange}
      />
    );
  }

  return (
    <InventoryDesktopLayout<TRow, TState, TFilters>
      title={title}
      totalCount={totalCount}
      state={state}
      topRight={topRight}
      filters={filters}
      activeFilters={activeFilters}
      clearFiltersPatch={clearFiltersPatch}
      renderFilterPanel={renderFilterPanel}
      onStatePatch={onStatePatch}
      columns={columns}
      rows={rows}
      getRowId={getRowId}
      ariaLabel={ariaLabel}
      loadingState={loadingState}
      onLoadMore={onLoadMore}
      sortDescriptor={sortDescriptor}
      onSortChange={onSortChange}
    />
  );
};
